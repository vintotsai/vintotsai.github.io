<template>
  <div class="qqbk-page">
    <!-- 放大图片 -->
    <enlarge-img v-if="isShowBigImg" @close-big-img="closeBigImg" :imgSrc="imgSrc"></enlarge-img>
    <!-- 轮播卡片 -->
      ...
      <swiper-slide v-for="(item, index) in qqbkList" :key="index">
        <div class="card" v-show="!isShowBigImg">
          <div class="img-container swiper-zoom-container">
            <!-- 爆品图片 -->
            <img :src="require('./imgs/' + item.id + '.jpg')" :alt="item.id" @click="enlargeMe($event)">
          </div>
          ...
          <div class="btn">
            <a :href="require('./imgs/' + item.id + '.jpg')" :download="item.id"></a>
...
export default {
  ...
  data() {
    return {
      swiperOpts: {
        initialSlide: window.location.hash.split("#")[1], //取hash值 用于哈希链接
        slidesPerView: "auto",
        spaceBetween: 5,
        centeredSlides: true,
        hashnav: true,
